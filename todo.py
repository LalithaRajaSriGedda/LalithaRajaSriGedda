...
def add_task():
    ...
    if len(task_string)==0:
        messagebox.showinfo('Error','Feild is empty')
    else:
...
def retrive_database():
    while(len(tasks)!=0):
        tasks.pop()
    for row in the_cursor.execute("select title frok tasks")
    tasks.append(row[0])
if __name__=="__main__":
    guiWindow=tk.Tk()
    guiWindow.title("TO_Do list manager")
    guiWindow.geometry("500*500+750+250")
    guiWindow.resizable(0,0)
    guiWindow.configure(bg="#FAEBD7")
    the_connection=sql.connect("listOfTasks.db")
    the-cursor=the_connetion.cursor()
...
header_label=ttk.Label(header_frame,text="The To-Do List",font=("Brush Script MT","30"),background="FAEBD7",foreground="#8B4513")
...
task_feild=ttk.Entry(functions_frame,font=("consolas","12"),width=18,background="#FFF8DC",foreground="#A52A2A")
task_feild.place(x=30,y=80)
add_button=ttk.Button(functions_frame,text="Add task",width=24,command=add_task)
del_buttton=ttk.Button(functions_frame,text="Delete tasks",width=24,command="delete_task")
del_all_buttton=ttk.Button(functions_frame,text="Delete all tasks",width=24,command="delete_all_task")
